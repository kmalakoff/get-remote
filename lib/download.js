...
module.exports = function download(src, dest, options, callback) {
  ...
  req.on('response', function (res) {
    ...
    if (options.extract) {
      ...
      fastExtract(res, dest, assign({}, options, { filename: filename, extension: extension, writeStreamOptions: options.writeStreamOptions || {} }), callback);
    } else {
      res = res.pipe(fsWriteStreamAtomic(entry.fullPath, options.writeStreamOptions || {}));
